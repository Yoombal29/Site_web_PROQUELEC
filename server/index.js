...
function sendSseEvent(event, data) {
    ...
    if (discCount > 0) {
        console.log(`[SSE] Cleaned up ${discCount} disconnected client(s). Active: ${sseStats.activeConnections}`);
    }
}
...
app.get('/api/events', (req, res) => {
    ...
    console.log(`[SSE] Client connected. Total connections: ${sseStats.totalConnections}, Active: ${sseStats.activeConnections}`);
...
const JWT_SECRET = process.env.JWT_SECRET || process.env.VITE_JWT_SECRET || 'dev-secret-key';
...
// RBAC: Granular Permission Middleware
function requirePermission(permissionName) {
    return async (req, res, next) => {
        try {
            ...
            // 1. Role-based permissions (role_permissions)
            // 2. User-specific permissions (user_permissions)
            ...
            const permissionCheck = await pool.query(`
                SELECT EXISTS(
                    ...
                    -- Check user-specific grants (override)
...
app.post('/api/inspections/suggest-checklist', authenticateToken, async (req, res) => {
    try {
        ...
        const detectedType = installationType || project?.technical_info?.installation_type || 'Résidentiel';
                ...
                categories: [
                    {
                        ...
                        checks: [
                            ...
                            { label: 'Protection contre les surintensités (disjoncteurs/fusibles)', critical: true }
                        ]
                    },
                    {
...
app.post('/api/storage/upload', (req, res) => {
    upload.single('file')(req, res, async function (err) {
        if (err instanceof multer.MulterError) {
            ...
            return res.status(500).json({ error: 'Erreur téléversement', message: err.message });
        } else if (err) {
        ...
        try {
            // Insert into DB with Project Context
            const dbRes = await pool.query(
                `INSERT INTO public.media_files 
                 (file_name, file_path, mime_type, file_size, uploaded_at, project_id, doc_category) 
            ...
            try { sendSseEvent('media:uploaded', { id: dbRes.rows[0].id, url: fileUrl, filename: req.file.filename }); } catch (e) { console.warn('SSE broadcast failed', e); }

        } catch (dbErr) {
            console.error('DB Insert Error during upload:', dbErr);
            // Fallback
            res.json({
...
app.post('/api/storage/upload', authenticateToken, (req, res) => {
    upload.single('file')(req, res, async function (err) {
        if (err instanceof multer.MulterError) {
        ...
        try {
            ...
            // Validate project_id UUID format if needed, OR just pass it (PG will error if invalid UUID but let's assume it's OK or NULL)

            const dbRes = await pool.query(
                `INSERT INTO public.media_files (file_name, file_path, mime_type, file_size, uploaded_at, project_id) 
                 VALUES ($1, $2, $3, $4, NOW(), $5) RETURNING *`,
                [req.file.originalname, req.file.filename, req.file.mimetype, req.file.size, project_id || null]
            );

            console.log(`[STORAGE] File uploaded: ${req.file.originalname} (${req.file.size} bytes)`);
            res.json(dbRes.rows[0]);
            try { sendSseEvent('media:uploaded', dbRes.rows[0]); } catch (e) { }

        } catch (dbErr) {
            console.error('[STORAGE] DB Insert Failed:', dbErr);
...
app.put('/api/storage/files/:id/rename', authenticateToken, async (req, res) => {
    try {
        ...
        try { sendSseEvent('media:renamed', updateResult.rows[0]); } catch (e) { console.warn('SSE broadcast failed (media:renamed)', e); }
    } catch (err) {
...
function authenticateToken(req, res, next) {
    ...
    jwt.verify(token, JWT_SECRET, (err, user) => {
        if (err) {
            console.error(`[AUTH] Token Verification Error for ${req.originalUrl}:`, err.message);
...
function requireAdmin(req, res, next) {
    if (!req.user || req.user.role !== 'admin') {
        console.warn(`[AUTH] Access denied for user ${req.user?.email || 'unknown'} to ${req.originalUrl}`);
...
app.post('/api/ai/generate-visual', async (req, res) => {
    try {
        ...
        res.json({
            ...
            metadata: { model: "NodeJS-Fallback", style: "error-handling" }
        });
    }
});
...
app.get('/api/chats/:sessionId/messages', authenticateToken, async (req, res) => {
    try {
        ...
        const check = await pool.query('SELECT 1 FROM chats WHERE id = $1 AND user_id = $2', [req.params.sessionId, req.user.id]);
        if (check.rowCount === 0) {
            console.warn(`[CHAT-API] Unauthorized access attempt to session: ${req.params.sessionId} by user: ${req.user.id}`);
            return res.status(403).send('Unauthorized');
        }

        const result = await pool.query('SELECT * FROM messages WHERE chat_id = $1 ORDER BY created_at ASC', [req.params.sessionId]);
...
app.post('/api/chats/:sessionId/messages', authenticateToken, async (req, res) => {
    try {
        ...
        const check = await pool.query('SELECT 1 FROM chats WHERE id = $1 AND user_id = $2', [req.params.sessionId, req.user.id]);
        if (check.rowCount === 0) {
            console.warn(`[CHAT-API] Unauthorized message addition attempt to session: ${req.params.sessionId} by user: ${req.user.id}`);
            return res.status(403).send('Unauthorized');
        }
...
app.put('/api/site-settings', authenticateToken, async (req, res) => {
    try {
        const {
            ...
            page_sections = {},
            audience_section_title = null, audience_section_subtitle = null,
            audience_title_electrician = null, audience_subtitle_electrician = null, audience_desc_electrician = null,
            audience_title_company = null, audience_subtitle_company = null, audience_desc_company = null,
            audience_title_member = null, audience_subtitle_member = null, audience_desc_member = null
        } = req.body;
        const result = await pool.query(
...
app.put('/api/theme-settings', authenticateToken, async (req, res) => {
    try {
        ...
        res.json(result.rows[0]);
        try { sendSseEvent('theme:updated', result.rows[0]); } catch (e) { console.warn('SSE broadcast failed (theme:updated)', e); }
    } catch (err) {
...
app.post('/api/cache/purge', authenticateToken, async (req, res) => {
    try {
        ...
        if (process.env.CDN_PURGE_URL) {
            try {
                const fetchRes = await fetch(process.env.CDN_PURGE_URL, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${process.env.CDN_PURGE_KEY || ''}` },
        ...
        try { sendSseEvent('cache:purged', { paths }); } catch (e) { console.warn('SSE broadcast failed (cache:purged)', e); }
    ...
    try {
        ...
        try {
            await pool.query(
                ...
                ['QUERY_ERROR', JSON.stringify({ error: err.message, code: err.code, query: text, params })]
            );
        } catch (logErr) {
...
app.post('/api/menu-items', authenticateToken, async (req, res) => {
    const { title, url, menu_order, parent_id, is_active, menu_type, target, icon, label } = req.body;
    ...
    await executeQuery(res,
        'INSERT INTO public.menu_items (title, url, menu_order, parent_id, is_active, menu_type, target, icon, label) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *',
        params
    );
});
...
app.put('/api/menu-items/:id', authenticateToken, async (req, res) => {
    ...
    const fields = { title, url, menu_order, parent_id, is_active, menu_type, target, icon, label };
...
app.post('/api/blog-categories', authenticateToken, async (req, res) => {
    const { name } = req.body;
    await executeQuery(res, 'INSERT INTO public.blog_categories (name) VALUES ($1) RETURNING *', [name]);
});

app.put('/api/blog-categories/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { name } = req.body;
    await executeQuery(res, 'UPDATE public.blog_categories SET name=$1 WHERE id=$2 RETURNING *', [name, id]);
});
...
app.post('/api/audits', authenticateToken, async (req, res) => {
    const { site_name, location, inspector_id, findings, compliance_score, recommendations } = req.body;
    await executeQuery(res,
...
app.post('/api/training-registrations', authenticateToken, async (req, res) => {
    const { training_id, participant_name, participant_email, participant_phone, company_name, special_requirements } = req.body;
    try {
        const result = await pool.query(`
    ...
    RETURNING *
        `, [training_id, req.user.id, participant_name, participant_email, participant_phone, company_name, special_requirements]);
        res.json(result.rows[0]);
    } catch (err) {
...
app.post('/api/site-assets', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res,
        ...
        [
            title, description, category || 'Général', asset_type || 'PDF', file_size,
            file_url, preview_url, is_premium || false, price_fcfy || 0,
            monetization_active || false, metadata || {}
        ]
    );
});
...
app.put('/api/site-assets/:id', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res,
        ...
        [
            title, description, category, asset_type, file_size,
            file_url, preview_url, is_premium, price_fcfy,
            monetization_active, metadata, id
        ]
    );
});
...
app.get('/api/gallery-items', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM public.gallery_items WHERE is_active = true ORDER BY display_order ASC');
        res.json(result.rows);
    } catch (err) {
...
app.get('/api/admin/gallery-items', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM public.gallery_items ORDER BY display_order ASC');
        res.json(result.rows);
    } catch (err) {
        console.error('Error fetching gallery items:', err);
        res.status(500).json({ error: err.message });
...
app.post('/api/gallery-items', authenticateToken, async (req, res) => {
    const { title, description, url, type, category, tags, hotspots, display_order, is_active } = req.body;
    await executeQuery(res,
...
app.post('/api/media-files', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res,
        ...
        [file_name, file_path, file_type, file_size, mime_type, alt_text, req.user.id, project_id, folder_path || '/', status || 'draft', metadata || {}]
    );
});
...
app.post('/api/upload', authenticateToken, upload.single('file'), async (req, res) => {
    try {
        ...
        const result = await pool.query(
            ...
            [req.file.originalname, filePath, category || 'other', req.file.size, req.file.mimetype, req.user.id, project_id, folder_path || '/', status || 'draft']
        );
...
app.post('/api/professional-training', authenticateToken, async (req, res) => {
    const { title, description, duration_hours, level, price, max_participants, instructor_name, location, equipment_provided, prerequisites, learning_objectives } = req.body;
    await executeQuery(res,
        ...
        [title, description, duration_hours, level, price, max_participants, instructor_name, location, equipment_provided, prerequisites, learning_objectives]
    );
});
...
app.put('/api/professional-training/:id', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res,
        ...
        [title, description, duration_hours, level, price, max_participants, instructor_name, location, equipment_provided, prerequisites, learning_objectives, id]
    );
});
...
const initDB = async () => {
        ...
        const initialPages = [
            {
                ...
                meta_description: 'Découvrez PROQUELEC, l\'organisme national de référence pour la qualité et la sécurité des installations électriques au Sénégal.',
            ...
            },
            {
                title: 'Nos Activités',
                slug: 'activities',
                content: '<h2>Contrôle de conformité</h2><p>Vérification complète et certifications des installations conformes aux normes.</p><h2>Labellisation</h2><p>Attribution du label de qualité après audit rigoureux.</p><h2>Formations</h2><p>Programmes de formation certifiants pour électriciens et techniciens.</p>',
                meta_description: 'Découvrez toutes nos activités : contrôle de conformité, labellisation, formations, audits énergétiques.',
            ...
            },
            {
                ...
                slug: 'labels',
                content: '<h2>Le Label PROQUELEC</h2><p>Le Label PROQUELEC est une marque de qualité qui garantit l\'excellence des installations électriques.</p>',
                meta_description: 'Découvrez le Label PROQUELEC, marque de qualité pour les professionnels de l\'électricité au Sénégal.',
            ...
            },
            {
                ...
                slug: 'certifications',
                content: '<h2>Programme de Certification</h2><p>Nos certifications valident vos compétences et garantissent votre expertise professionnelle.</p>',
                meta_description: 'Découvrez nos programmes de certification pour valider vos compétences en électricité.',
            ...
            },
            {
                title: 'Documentation',
                slug: 'documents',
                content: '<h2>Centre de Documentation</h2><p>Accédez à notre bibliothèque complète de documents techniques et normes.</p>',
                meta_description: 'Téléchargez nos documents techniques, guides professionnels et ressources.',
            ...
            },
            {
                title: 'Évènements',
                slug: 'events',
                content: '<h2>Calendrier des Évènements</h2><p>Découvrez nos conférences, ateliers et webinaires.</p>',
                meta_description: 'Découvrez nos événements : conférences, ateliers et formations sur l\'électricité au Sénégal.',
            ...
            },
            {
                title: 'Formations',
                slug: 'formations',
                content: '<h2>Centre de Formation</h2><p>Développez vos compétences avec nos formations professionnelles.</p>',
            ...
            },
            {
                title: 'Expertises Techniques',
                slug: 'expertises-techniques',
                content: '<h2>Expertises Techniques</h2><p>Nos domaines d\'expertise au service de votre sécurité.</p>',
                meta_description: 'Découvrez les expertises techniques de PROQUELEC en installations électriques.',
            ...
            },
            {
                title: 'Formations PROQUELEC',
                slug: 'formations-proquelec',
                content: '<h2>Formations PROQUELEC</h2><p>Découvrez nos programmes de formation avancés.</p>',
            ...
            },
            {
                ...
                slug: 'contact',
                content: '<h2>Contact & Localisation</h2><p>Notre équipe d\'experts est à votre entière disposition pour répondre à vos questions techniques.</p>',
                meta_description: 'Contactez PROQUELEC pour vos besoins d\'audit, formation ou certification électrique.',
            ...
            },
            {
                ...
                slug: 'legal',
                content: '<h2>Mentions Légales</h2><p>Responsable du site, hébergement et confidentialité.</p>',
            ...
            const defaultSlides = [
                {
                    ...
                    description: "Expert en installations électriques au Sénégal depuis 1995. Nous garantissons la sécurité, la qualité et la conformité de vos projets électriques.",
                ...
                },
                {
                    ...
                    description: "Saviez-vous que 93% des installations au Sénégal ne sont pas conformes ? Nos experts certifiés réalisent des audits approfondis.",
                    background_url: "https://images.unsplash.com/photo-1581092160562-40aa08e78837?q=80&w=2000&auto=format&fit=crop",
    ...
    ('Efficacité Énergétique', 'Audits énergétiques et optimisation des installations', 'Zap', '/contact', ARRAY['Économies garanties', 'Solutions durables', 'ROI calculé'], 2)
        ...
        const adminEmail = 'experimental@proquelec.com';
...
app.post('/api/home-slides', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res, `INSERT INTO public.home_slides
...
app.put('/api/home-slides/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { badge, title, subtitle, description, background_url, cta_text, cta_link, secondary_cta_text, secondary_cta_link, display_order } = req.body;
    await executeQuery(res, `UPDATE public.home_slides 
...
app.post('/api/home-stats', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res, 'INSERT INTO public.home_stats (label, value, icon_name, description, is_warning, display_order) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
...
app.put('/api/home-stats/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { label, value, icon_name, description, is_warning, display_order } = req.body;
    await executeQuery(res, 'UPDATE public.home_stats SET label=$1, value=$2, icon_name=$3, description=$4, is_warning=$5, display_order=$6 WHERE id=$7 RETURNING *',
...
app.post('/api/admin/db/query', authenticateToken, async (req, res) => {
    const { query } = req.body;
    console.log(`[SQL EXEC] Requête reçue : ${query?.substring(0, 100)}...`);
    try {
        if (!query || query.trim() === '') {
        ...
        // Handle multi-statements (result is an array)
...
app.get('/api/intelligence/insights', authenticateToken, async (req, res) => {
    try {
        ...
        const certs = await pool.query('SELECT type, status, COUNT(*) as count FROM public.certifications GROUP BY type, status');
        insights.certifications = certs.rows;

        // 2. Audit Compliance Heatmap (Simplified by region)
        const audits = await pool.query('SELECT location, AVG(compliance_score) as avg_score, COUNT(*) as volume FROM public.audits GROUP BY location');
        insights.audits = audits.rows;

        // 3. Network Density
        const network = await pool.query('SELECT region, COUNT(*) as pros FROM public.electricians_network WHERE is_verified = true GROUP BY region');
...
app.get('/api/admin/stats', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({
            ...
            totalDocuments: counts[3].status === 'fulfilled' ? parseInt(counts[3].value.rows[0].count) : 0,
            totalAiRequests: counts[4].status === 'fulfilled' ? parseInt(counts[4].value.rows[0].count) : 0
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.post('/api/home-services', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res, 'INSERT INTO public.home_services (title, description, icon_name, link, features, display_order) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
...
app.put('/api/home-services/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { title, description, icon_name, link, features, display_order } = req.body;
    await executeQuery(res, 'UPDATE public.home_services SET title=$1, description=$2, icon_name=$3, link=$4, features=$5, display_order=$6 WHERE id=$7 RETURNING *',
...
app.post('/api/testimonials', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res, 'INSERT INTO public.testimonials (name, role, content, rating, avatar_url) VALUES ($1, $2, $3, $4, $5) RETURNING *',
...
app.put('/api/testimonials/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    const { name, role, content, rating, avatar_url } = req.body;
    await executeQuery(res, 'UPDATE public.testimonials SET name=$1, role=$2, content=$3, rating=$4, avatar_url=$5 WHERE id=$6 RETURNING *',
...
app.post('/api/form-submissions', async (req, res) => {
    try {
        ...
        try { sendSseEvent('page:created', result.rows[0]); } catch (e) { console.warn('SSE broadcast failed (page:created)', e); }
    } catch (error) {
...
app.get('/api/search', async (req, res) => {
    try {
        ...
        if (typeList.includes('page')) {
            ...
            allResults = [...allResults, ...rows.map(r => ({
                ...r,
...
app.delete('/api/pages/:id', authenticateToken, async (req, res) => {
    try {
        ...
        res.json({ success: true });
        try { sendSseEvent('page:deleted', { id: req.params.id }); } catch (e) { console.warn('SSE broadcast failed (page:deleted)', e); }
    } catch (error) {
        handleAppError(error, res);
    }
});
...
app.get('/api/admin/page-versions/:id/:version', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const { id, version } = req.params;
        const { rows } = await pool.query('SELECT * FROM public.page_versions WHERE page_id = $1 AND version = $2', [id, version]);
...
app.get('/api/cms/plugins', async (req, res) => {
    try {
        ...
        res.json([]); // Fail gracefully for plugins
    }
});
...
app.post('/api/email/welcome', async (req, res) => {
    console.log('[MAIL] Envoi email bienvenue à:', req.body.email);
    res.json({ success: true });
});

app.post('/api/email/formation-confirmation', async (req, res) => {
    console.log('[MAIL] Confirmation formation:', req.body.formationName, 'pour', req.body.email);
    res.json({ success: true });
});

app.post('/api/email/certification-notification', async (req, res) => {
    console.log('[MAIL] Notification certification:', req.body.certificationName, 'pour', req.body.email);
    res.json({ success: true });
});
...
app.post('/api/ai/seo-analyze', authenticateToken, requireAdmin, async (req, res) => {
    try {
        ...
        if (!apiKey) {
            return res.status(503).json({
                success: false,
                message: 'Le système IA (Google Gemini) n\'est pas encore configuré (Clé GEMINI_API_KEY manquante dans .env).',
                code: 'AI_CONFIG_MISSING'
            });
        }
        ...
        try {
            await pool.query(
                `INSERT INTO public.ai_requests_log (user_id, endpoint, prompt, response, created_at) 
                 VALUES ($1, $2, $3, $4, NOW())`,
                [req.user.id, '/api/ai/seo-analyze', (systemPrompt + userPrompt).substring(0, 500), JSON.stringify(seoData)]
            );
        } catch (logErr) {
...
app.get('/api/admin/system/status', authenticateToken, requireAdmin, async (req, res) => {
    try {
        const statuses = {
            backend: { id: 'node', name: "Serveur Principal", status: "online", port: port, type: 'Node.js' },
            ai: { id: 'python', name: "Cerveau IA Expert", status: pythonProcess ? "online" : "offline", port: 8002, type: 'Python/Haystack' },
...
app.post('/api/electrical-certifications', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res,
        ...
        [name, code, description, validity_period, required_training_hours, certification_body, cost, requirements, is_active !== false]
    );
});
...
app.put('/api/electrical-certifications/:id', authenticateToken, async (req, res) => {
    ...
    await executeQuery(res,
        ...
        [name, code, description, validity_period, required_training_hours, certification_body, cost, requirements, is_active, id]
    );
});
...
// --- ELECTRO-GED 4.0: PROJECT MANAGEMENT ROUTES ---

// 1. LIST PROJECTS
app.get('/api/projects', authenticateToken, async (req, res) => {
    try {
        // Filter by user role if needed (e.g. only see own projects unless admin)
        // For now, let's say admins see all, installateurs/clients see theirs
        // But simplifying: show all for authenticated users for demo
...
app.post('/api/projects', authenticateToken, requirePermission('projects.create'), async (req, res) => {
    try {
        ...
        // Generate reference (Simple logic for now: PQ-YEAR-RANDOM)
        ...
        console.log(`[PROJECTS] Created: ${result.rows[0].title} (${result.rows[0].id})`);
        res.status(201).json(result.rows[0]);
    } catch (err) {
...
app.get('/api/projects/:id/documents', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        // Join media_files with project_id
        const result = await pool.query(
...
// 5. UPDATE PROJECT (Status, Score, etc.)
const crypto = require('crypto'); // Ensure crypto is imported

// 5. UPDATE PROJECT (with Audit Trail V2)
app.put('/api/projects/:id', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // 1. Get Current State (Snaphost Before)
        ...
        // 3. Compute Diff (Simple JSON comparison)
        const changes = {};
        ...
        // 4. Create Audit Log (Authority Record)
        if (Object.keys(changes).length > 0) {
            const action = 'UPDATE_PROJECT';
            const timestamp = new Date().toISOString();
            // Cryptographic Signature: SHA256(id + action + timestamp + changes)
...
// 6. GET PROJECT AUDIT TRAIL (Authority History)
app.get('/api/projects/:id/audit', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query(
...
// 7. PROJECT STATE MACHINE (Authority Lifecycle Management)
// Enforces strict regulatory transitions: draft → submitted → under_review → validated/rejected
app.post('/api/projects/:id/transition', authenticateToken, requirePermission('projects.transition'), async (req, res) => {
    const client = await pool.connect();
    try {
        ...
        // 2. Define State Machine (Allowed Transitions)
        const STATE_MACHINE = {
            draft: {
                allowed_next: ['submitted'],
                required_roles: ['installer', 'admin']
            },
            submitted: {
                allowed_next: ['under_review', 'draft'],
                required_roles: ['admin', 'authority']
            },
            under_review: {
        ...
        if (!currentState.allowed_next.includes(new_status)) {
            ...
            return res.status(403).json({
                ...
                allowed: currentState.allowed_next
            });
        }
        ...
        if (!currentState.required_roles.includes(userRole) && userRole !== 'admin') {
            ...
            return res.status(403).json({
                ...
                required_roles: currentState.required_roles,
                your_role: userRole
            });
        }
        ...
        const updateRes = await client.query(
            `UPDATE public.projects 
             SET regulatory_status=$1, updated_at=NOW() 
             WHERE id=$2 RETURNING *`,
            [new_status, id]
        );
        const updatedProject = updateRes.rows[0];

        // 6. Log Transition in Audit Trail (Authority Record)
...
// --- ELECTRO-GED 4.0: INSPECTION MODULE ROUTES ---

// 1. GET CHECKLISTS (Templates)
...
// 2. GET CHECKLIST ITEMS (Questions)
...
// 3. LIST INSPECTIONS (By Project)
app.get('/api/projects/:id/inspections', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const result = await pool.query(
...
// 4. CREATE INSPECTION with RESULTS (Atomic)
app.post('/api/inspections', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        // Gérer le cas checklist_id = 'suggested' (checklist IA, pas un UUID)
        ...
        if (results && Array.isArray(results)) {
            ...
            // Si checklist IA, stocker les results dans checklist (JSONB) au lieu de inspection_results
            if (checklist_id === 'suggested') {
                // Transformer les results en structure checklist
                const checklistData = {
                    type: 'suggested',
                    items: results.map(r => ({
                        id: r.item_id,
                ...
                // Cas classique : checklist avec UUID
            ...
            // 3. ADVANCED SCORE CALCULATION (Weighted & Section-based)
            if (results.length > 0) {
                // A. Calculate Overall Score for THIS Inspection
                // Simple average for now, but could use weights from checklist_items
                ...
                // B. Calculate Dynamic Breakdown for the PROJECT (Authority View) - SKIP si checklist IA
                if (checklist_id !== 'suggested') {
                    // Aggregates ALL inspections for this project to give a live compliance map
                    ...
                    const breakdown = breakdownRes.rows.map(row => ({
                        domain: row.section || 'Général',
                        score: Number(row.score),
                        weight: 1 // Default weight, could be dynamic later
                    }));

                    // C. Recalculate Project Overall Score (Average of all inspections)
                    ...
                    // D. Update Project with Authority Data
                    const complianceDetails = {
                        overall_score: newProjectScore,
                        breakdown: breakdown,
                        last_evaluated_at: new Date().toISOString(),
                        ai_model_version: "NS-01-001-v3.2" // Hardcoded for now, dynamic later
                    };

                    await client.query(
                        `UPDATE public.projects 
                     SET compliance_score=$1, 
                         compliance_details=$2 
                     WHERE id=$3`,
...
app.get('/api/inspections/:id', authenticateToken, async (req, res) => {
    try {
        ...
        // 2. Get Results 
        // Cas A: Checklist intelligente/suggérée (résultats stockés dans la colonne JSONB 'checklist')
        if (inspection.checklist && inspection.checklist.items) {
            inspection.results = inspection.checklist.items.map(item => ({
                ...
                checkpoint_label: item.id || item.label || item.question, // Fallback labels
                checkpoint_category: inspection.checklist.type || 'Suggéré',
                checkpoint_description: ''
            }));
        } else {
            // Cas B: Checklist standard (UUID, résultats dans inspection_results)
            const resultsRes = await pool.query(
                `SELECT r.*, 
                        ci.section as checkpoint_category, 
                        ci.question as checkpoint_label,
                        ci.description as checkpoint_description,
...
// 6. DELETE AUDIT LOG
app.delete('/api/audit-logs/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/api/inspections/:id/results', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { results } = req.body; // Array of { item_id, value, is_compliant, comment }

        // Transaction could be safer here
        const client = await pool.connect();
        try {
            await client.query('BEGIN');
            ...
            // Auto-update score logic here (simplified)
            // ...
...
app.post('/api/office/documents', authenticateToken, async (req, res) => {
    try {
        ...
        const filePath = path.join('office', filename);
        const fullPath = path.join(__dirname, 'uploads', 'office', filename);
        ...
        // Save Metadata to DB (media_files)
        // We use a custom mime type to distinguish office docs
        const mimeType = type === 'spreadsheet' ? 'application/vnd.proquelec.sheet' :
            type === 'presentation' ? 'application/vnd.proquelec.presentation' :
                'application/vnd.proquelec.document';

        const dbRes = await pool.query(
            `INSERT INTO public.media_files (file_name, file_path, mime_type, file_size, uploaded_at) 
             VALUES ($1, $2, $3, $4, NOW()) RETURNING *`,
            [title, filePath, mimeType, Buffer.byteLength(JSON.stringify(content))]
        );

        console.log(`[OFFICE] Document Created: ${title} (${dbRes.rows[0].id})`);
        res.json({ id: dbRes.rows[0].id, ...dbRes.rows[0] });

    } catch (err) {
        console.error('[OFFICE] Create Error:', err);
        res.status(500).json({ error: 'Failed to create document', details: err.message });
    }
});

// 2. UPDATE DOCUMENT
app.put('/api/office/documents/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { title, content } = req.body;

        // Get file path from DB
        const dbRes = await pool.query('SELECT * FROM public.media_files WHERE id=$1', [id]);
        if (dbRes.rowCount === 0) return res.status(404).json({ error: 'Document not found' });
        ...
        // Update Metadata in DB (size, title, updated_at if column exists)
        // Here we just update title/size
        await pool.query(
            'UPDATE public.media_files SET file_name=$1, file_size=$2 WHERE id=$3',
            [title, Buffer.byteLength(JSON.stringify(content)), id]
        );
...
// 3. GET DOCUMENT
app.get('/api/office/documents/:id', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (fs.existsSync(fullPath)) {
            ...
            res.json({
                id: doc.id,
                title: doc.file_name,
                type: doc.mime_type, // Map mime to simple type if needed
                content: JSON.parse(content),
                metadata: doc.metadata || {} // Assuming metadata column exists or empty
            });
        } else {
            // File missing but DB entry exists
            res.json({
                id: doc.id,
                title: doc.file_name,
                content: null, // Empty doc
                error: 'File content missing on disk'
            });
        }

    } catch (err) {
        console.error('[OFFICE] Read Error:', err);
        res.status(500).json({ error: 'Failed to read document' });
    }
});


// Consolidated AI routes handled at line 845+

// Consolidated Vision and Image routes handled at line 860+
...
app.post('/api/observatoire/sync/trigger', authenticateToken, requireAdmin, async (req, res) => {
    // Note: Dans une vraie implémentation, on appellerait runSyncCycle() exposé
    // Pour l'instant, c'est géré par l'intervalle, mais on pourrait l'exposer
