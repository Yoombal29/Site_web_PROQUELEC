...
async function findAllEvents() {
  const result = await pool.query('SELECT * FROM public.events ORDER BY date ASC');
  return result.rows;
}

async function createEvent(data) {
  const { title, date, location, details, organizer_id, image_url, status, organizer_type } = data;
  const result = await pool.query(
    `INSERT INTO public.events
          (title, date, location, details, created_at, organizer_id, image_url, status, organizer_type)
...
async function updateEvent(id, data) {
  const { title, date, location, details, image_url, status, organizer_type } = data;
  const result = await pool.query(
    `UPDATE public.events
            SET title = COALESCE($1, title),
                ...
                organizer_type = COALESCE($7, organizer_type)
          WHERE id = $8
          RETURNING *`,
    [
      title || null,
...
async function findEventWithOrganizer(eventId) {
  const result = await pool.query(
    `SELECT e.*, u.email as organizer_email, u.name as organizer_name
...
async function findRegistrationsByEvent(eventId) {
  const result = await pool.query(
    'SELECT * FROM public.event_registrations WHERE event_id = $1 ORDER BY created_at DESC',
    [eventId],
  );
  return result.rows;
}
...
async function createEventRegistration(data) {
  ...
  const result = await pool.query(
    `INSERT INTO public.event_registrations (event_id, name, email, phone, company, message)
         VALUES ($1, $2, $3, $4, $5, $6)
         RETURNING *`,
    [event_id, name, email, phone || null, company || null, message || null],
  );
  return result.rows[0];
}
...
async function updateRegistrationStatus(id, status) {
  const result = await pool.query(
    `UPDATE public.event_registrations SET status = $1 WHERE id = $2 RETURNING *`,
    [status, id],
  );
  return result.rows[0];
}

async function countRegistrationsByEvent(eventId) {
  const result = await pool.query(
    `SELECT status, COUNT(*)::int FROM public.event_registrations WHERE event_id = $1 GROUP BY status`,
    [eventId],
  );
  return result.rows;
}
...
async function findAllContacts() {
  const result = await pool.query(
    'SELECT * FROM public.contact_requests ORDER BY submitted_at DESC',
  );
  return result.rows;
}
