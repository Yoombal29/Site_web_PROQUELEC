...
function mountAiFeaturesRoutes(app, pool) {
  ...
  app.post('/api/ai/diagnostic', authenticateToken, requireAdmin, async (req, res) => {
    ...
    try {
      ...
      let pingUrl = '',
        pingOptions = { method: 'GET', headers: {} };
  ...
  app.post('/api/ai/scan-compliance', authenticateToken, requireAdmin, async (req, res) => {
    try {
      ...
      const parsedData = jsonMatch
        ...
        : { error: 'Erreur de formatage IA', raw: responseText };
  ...
  app.post('/api/ai/seo-analyze', authenticateToken, requireAdmin, async (req, res) => {
    try {
      ...
      try {
        await pool.query(
          'INSERT INTO public.ai_requests_log (user_id, endpoint, prompt, response, created_at) VALUES ($1, $2, $3, $4, NOW())',
          [
            req.user.id,
            '/api/ai/seo-analyze',
      ...
      res.status(500).json({ success: false, message: error.message, code: 'AI_SEO_FAIL' });
    }
  });
