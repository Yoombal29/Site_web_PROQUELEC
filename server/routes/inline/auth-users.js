...
function mountAuthUsersRoutes(app, pool, deps) {
  ...
  // ============================================
  // USER PERMISSIONS (for the connected user)
  // ============================================
  app.get('/api/user/permissions', authenticateToken, async (req, res) => {
    try {
      ...
      if (userRole === 'superadmin') {
        const allPerms = await pool.query('SELECT name FROM public.permissions ORDER BY name');
        return res.json({ permissions: allPerms.rows.map((r) => r.name) });
      }
      ...
      const result = await pool.query(
        `SELECT DISTINCT p.name
         ...
         WHERE (
           EXISTS (SELECT 1 FROM public.role_permissions rp WHERE rp.role = $1 AND rp.permission_id = p.id)
           OR
           EXISTS (SELECT 1 FROM public.user_permissions up WHERE up.user_id = $2 AND up.permission_id = p.id AND up.granted = true)
         )
         AND NOT EXISTS (
           SELECT 1 FROM public.user_permissions up WHERE up.user_id = $2 AND up.permission_id = p.id AND up.granted = false
         )
         ORDER BY p.name`,
        [userRole, userId],
      );

      res.json({ permissions: result.rows.map((r) => r.name) });
    } catch (err) {
