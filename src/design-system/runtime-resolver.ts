...
export function resolveTokenValue(value: unknown): unknown {
  if (typeof value !== 'string') return value;

  const m = value.match(colorPattern);
  if (m && colorTokens[m[1]]?.[m[2]]) {
...
export function resolveTokenStyle<T extends Record<string, unknown>>(style: T): T {
  if (!style) return style;
  const resolved = { ...style };
  for (const key of Object.keys(resolved)) {
    (resolved as any)[key] = resolveTokenValue((resolved as any)[key]);
  }
  return resolved;
}

export function useTokenResolvedStyle<T extends Record<string, unknown>>(style: T): React.CSSProperties {
  return resolveTokenStyle(style) as React.CSSProperties;
}
