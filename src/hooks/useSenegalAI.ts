...
import { searchNS01001, findArticle, NS01001Rule } from '@/data/ns01001-loader';
...
export function useSenegalAI() {
    ...
    const querySenegalEngine = async (userQuery: string): Promise<SenegalAIResponse> => {
        ...
        try {
            ...
            // 1. RECHERCHE DANS LE CORPUS ATOMISÉ (SUPABASE) - Haute Qualité
            const { data: dbArticles } = await (supabase as any)
                .from('normative_articles')
                .select('*, normative_books!inner(ref_code, title)')
                .ilike('content_exact', `%${userQuery.split(' ').slice(0, 2).join('%')}%`)
                .limit(3);
            ...
            if (dbArticles && dbArticles.length > 0) {
                dbArticles.forEach((art: any) => {
                    allArticles.push({
                        id: art.id,
                        titre: `${art.normative_books?.ref_code} - ${art.chapter_ref}`,
