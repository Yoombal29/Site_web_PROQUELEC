...
import { configApi } from '@/lib/ai-config.service';
...
const MODEL_LABELS: Record<string, string> = {
  lovable: 'Cœur Souverain PROQUELEC',
  ...
  openrouter: '300+ modèles (GPT, Claude, Gemini...)',
...
export default function AIProvidersPage() {
  ...
  const [expectedSecurityAnswer, setExpectedSecurityAnswer] = useState('CORAN');
  ...
  const [diagInfo, setDiagInfo] = useState<{
    ...
    data: unknown;
    isLoading: boolean;
  }>({ isOpen: false, provider: null, data: null, isLoading: false });
  ...
  const [providers, setProviders] = useState<AIProvider[]>([
    {
      id: 'lovable',
      name: 'Cœur Souverain PROQUELEC',
      model: MODEL_LABELS.lovable,
      key: 'LOCAL_NODE',
      status: 'configured',
      enabled: true,
  ...
  const [logs, setLogs] = useState<
    { id: string; msg: string; time: string; type: 'info' | 'success' | 'err' }[]
  >([]);
  ...
  const addLog = (msg: string, type: 'info' | 'success' | 'err' = 'info') => {
    setLogs((prev) =>
      [
        { id: Date.now().toString(), msg, time: new Date().toLocaleTimeString(), type },
        ...prev,
      ].slice(0, 10),
    );
  };
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      const enriched = parsed.map((p: any) => ({
        ...p,
        model: p.model || MODEL_LABELS[p.id] || 'Modèle inconnu',
      }));
    ...
    const interval = setInterval(() => {
      ...
      if (diff < 60) setUptime(`${diff}s`);
      else if (diff < 3600) setUptime(`${Math.floor(diff / 60)}m ${diff % 60}s`);
      else setUptime(`${Math.floor(diff / 3600)}h ${Math.floor((diff % 3600) / 60)}m`);
    }, 1000);

    return () => clearInterval(interval);
  }, []);

  const syncToStorage = async (updated) => {
    localStorage.setItem('ai_providers', JSON.stringify(updated));
    const configs = {};
    for (const p of updated) {
      if (p.key && p.id !== 'lovable') {
        configs['provider_' + p.id + '_key'] = p.key;
        configs['provider_' + p.id + '_enabled'] = p.enabled ? 'true' : 'false';
      }
    }
    try {
      ...
      addLog('Erreur sauvegarde serveur: ' + err.message, 'err');
    }
  };
  ...
  const lockKeys = () => {
    setIsUnlocked(false);
    toast({ title: 'Acces ROOT revoque' });
    addLog('Session ROOT terminée', 'info');
  };

  const changeSecurityAnswer = async () => {
    if (!newSecurityAnswer || newSecurityAnswer.length < 2) {
      toast({ title: 'Réponse trop courte (min. 2 caractères)', variant: 'destructive' });
      return;
    }
    if (newSecurityAnswer !== confirmSecurityAnswer) {
      toast({ title: 'Les réponses ne correspondent pas', variant: 'destructive' });
      return;
    }
    try {
      await configApi.save({ security_answer: newSecurityAnswer });
      setExpectedSecurityAnswer(newSecurityAnswer);
      setShowChangeSecurity(false);
      setNewSecurityAnswer('');
      setConfirmSecurityAnswer('');
      toast({ title: 'Réponse de vérification modifiée' });
      addLog('Réponse de vérification modifiée', 'success');
    } catch (err) {
  ...
  const changePassword = async () => {
    if (!newPassword || newPassword.length < 4) {
      toast({ title: 'Mot de passe trop court (min. 4 caractères)', variant: 'destructive' });
      return;
    }
    ...
    try {
      await configApi.save({ password: newPassword, admin_password: newPassword });
      setExpertPass(newPassword);
      setShowChangePass(false);
      setNewPassword('');
      setConfirmPassword('');
      toast({ title: 'Mot de passe modifié avec succès' });
      addLog('Mot de passe ROOT modifié', 'success');
    } catch (err) {
      toast({ title: 'Erreur lors de la sauvegarde', variant: 'destructive' });
      addLog('Erreur: ' + err.message, 'err');
    }
  ...
  const commitKey = async (pId: string, force = false) => {
    ...
    setProviders((prev) =>
      prev.map((p) => (p.id === pId ? { ...p, connectionStatus: 'checking' as const } : p)),
    );

    let result = { success: false, latency: 0 };
    if (!force) result = await testKey(pId, val);
    else result = { success: true, latency: 0 };
    ...
    if (result.success)
      toast({ title: force ? 'Forçage effectué' : `Signal établi (${result.latency}ms)` });
    else
      toast({
        title: 'Défaut de liaison',
        description: 'Le fournisseur a rejeté la requête.',
        variant: 'destructive',
      });
  };
  ...
  const toggleProvider = (pId: string, enabled: boolean) => {
    setProviders((prev) => {
      let next: AIProvider[] = prev.map((p) =>
        p.id === pId
          ? {
              ...p,
              enabled,
              connectionStatus: (enabled
                ? p.key
                  ? 'connected'
                  : 'disconnected'
                : 'disconnected') as AIProvider['connectionStatus'],
            }
          : p,
      );
      ...
      if (
        !enabled &&
        !next.some((p) => p.enabled && p.priority === 'expert' && p.id !== 'tavily')
      ) {
  ...
  const runDeepDiagnostic = async (p: AIProvider) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setProviders((prev) =>
          prev.map((x) => (x.id === p.id ? { ...x, lastDiag: result.diagnostics } : x)),
        );
      } else {
  ...
  const sovereignCount = activeProviders.filter(
    (p) => p.id === 'lovable' || p.id === 'ollama',
  ).length;
  const sovereigntyScore =
    activeProviders.length > 0 ? Math.round((sovereignCount / activeProviders.length) * 100) : 100;
  const availabilityScore =
    activeProviders.length > 0
      ? Math.round((connectedProviders.length / activeProviders.length) * 100)
      : 0;
  const avgLatency =
    connectedProviders.length > 0
      ? Math.round(
          connectedProviders.reduce((acc, p) => acc + (p.latency || 0), 0) /
    ...
    <div className="space-y-8 p-8 min-h-screen bg-[#020617] text-slate-200 relative overflow-hidden font-sans">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative z-10 border-b border-slate-800/50 pb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-400 flex items-center gap-2 italic">
                <ShieldCheck className="w-3 h-3 text-emerald-400" /> Gestionnaire de Souveraineté &
        ...
        <div className="flex items-center gap-4 bg-slate-900/40 p-1.5 rounded-2xl border border-slate-800">
          ...
          <Button
            onClick={() => {
              syncToStorage(providers);
              addLog('Configuration sauvegardee sur le serveur', 'success');
            }}
            className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl h-12 px-8 shadow-lg shadow-cyan-900/20 group"
          >
            ...
            <CardContent className="p-8 flex flex-col md:flex-row items-center gap-8">
              ...
              <div className="flex-1 space-y-2">
                <h3 className="text-xl font-bold text-white uppercase italic tracking-wider">
                  Identification Habilitée Requise
                </h3>
                <p className="text-sm text-slate-400 font-mono">
                  L'accès à la reconfiguration des passerelles satellite est restreint au personnel
                  habilité.
                </p>
              </div>
              <div className="flex w-full md:w-auto gap-2">
                ...
                <div className="relative flex-1 md:w-64">
                  <Input
                    type="text"
                    placeholder="QUESTION DE VÉRIFICATION..."
                    value={securityAnswer}
                    onChange={(e) => setSecurityAnswer(e.target.value)}
                    className="bg-slate-950/50 border-slate-800 h-14 pl-4 rounded-2xl"
                    onKeyDown={(e) => e.key === 'Enter' && unlockKeys()}
                  />
                </div>
                <Button
                  onClick={unlockKeys}
                  className="bg-white text-black font-black h-14 px-8 rounded-2xl hover:bg-cyan-400"
                >
                  ENTRER
                </Button>
              </div>
      ...
      {isUnlocked && (
        ...
        >
          <div className="flex items-center gap-2 bg-slate-900/40 border border-slate-800 rounded-2xl px-4 py-2">
            <span className="text-xs text-emerald-400 font-bold">🔓 ROOT</span>
            <Button
              variant="ghost"
              size="sm"
            ...
            >
              {showChangePass ? 'Annuler' : 'Changer mot de passe'}
            </Button>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setShowChangeSecurity(!showChangeSecurity)}
              className="text-xs text-slate-400 hover:text-white h-8"
            >
              {showChangeSecurity ? 'Annuler' : 'Changer vérification'}
            </Button>
            <Button
              variant="ghost"
              size="sm"
      ...
      {isUnlocked && showChangePass && (
        <motion.div
          initial={{ opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: 'auto' }}
          className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 space-y-3"
        >
          <h3 className="text-sm font-bold text-white">Changer le mot de passe ROOT</h3>
          <div className="flex gap-3">
            <Input
              ...
              className="bg-slate-950/50 border-slate-800 h-10 text-sm flex-1 rounded-xl"
            />
            <Input
              type="password"
            ...
            <Button
              ...
              className="bg-cyan-600 hover:bg-cyan-500 text-white h-10 px-6 rounded-xl"
            >
              Appliquer
            </Button>
          </div>
      ...
      {isUnlocked && showChangeSecurity && (
        <motion.div
          initial={{ opacity: 0, height: 0 }}
          animate={{ opacity: 1, height: 'auto' }}
          className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 space-y-3"
        >
          <h3 className="text-sm font-bold text-white">Changer la question de vérification</h3>
          <p className="text-xs text-slate-400">
            Réponse attendue pour déverrouiller : <strong>{expectedSecurityAnswer}</strong>
          </p>
          <div className="flex gap-3">
            <Input
              type="text"
      ...
      <Tabs defaultValue="souverain" className="w-full space-y-8 relative z-10">
        <TabsList className="bg-slate-900/50 border border-slate-800 p-1.5 h-16 rounded-[20px] backdrop-blur-md">
          <TabsTrigger
            value="souverain"
            className="rounded-[14px] data-[state=active]:bg-cyan-600 data-[state=active]:text-white h-full px-8 text-xs font-bold uppercase tracking-widest gap-2"
          >
        ...
        <TabsContent value="souverain" className="space-y-6">
          <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
            {providers
              .filter((p) => p.id === 'lovable' || p.id === 'ollama')
              .map((p) => (
                <ProviderCard
                  key={p.id}
                  p={p}
                  ...
                  onDiagnostic={runDeepDiagnostic}
                />
              ))}
          </div>
        </TabsContent>
        ...
        <div className="lg:col-span-2 space-y-8">
          <Card className="bg-slate-900/40 border-slate-800 rounded-3xl overflow-hidden backdrop-blur-sm">
            <CardHeader className="bg-slate-900/50 py-3 flex flex-row items-center justify-between border-b border-slate-800">
              ...
              <Badge variant="outline" className="text-[9px] border-cyan-500/30 text-cyan-500">
                LIVE RELAY
              </Badge>
            </CardHeader>
            <CardContent className="p-4 h-[200px] overflow-y-auto font-mono">
              {logs.length === 0 && (
                <p className="text-slate-600 text-[10px] italic">Attente de données...</p>
              )}
              {logs.map((log) => (
      ...
      <Dialog
        open={diagInfo.isOpen}
        onOpenChange={(o) => setDiagInfo((prev) => ({ ...prev, isOpen: o }))}
      >
            ...
            <div className="h-[600px] flex flex-col items-center justify-center p-12 space-y-8 relative">
              ...
              <div className="text-center space-y-2 relative">
                ...
                <p className="text-xs text-slate-500 font-mono italic">
                  Audit de la liaison satellite {diagInfo.provider?.id}...
                </p>
              </div>
              ...
              <div className="h-[700px] overflow-y-auto p-8 space-y-8 animate-in fade-in duration-500">
                <DialogHeader className="flex-row justify-between items-center mb-0">
                  <div className="space-y-1">
                    <DialogTitle className="text-3xl font-black text-white flex items-center gap-4">
                      ...
                      <Badge
                        className={cn(
                          'text-lg px-4 py-1 rounded-xl',
                          diagInfo.data.overallGrade === 'S'
                            ? 'bg-orange-500'
                            : diagInfo.data.overallGrade === 'A'
                              ? 'bg-cyan-500'
                              : 'bg-slate-700',
                        )}
                      >
                    ...
                    <DialogDescription className="text-slate-500 text-xs font-mono tracking-tighter uppercase">
                      Audit de Performance Atomique v2.5
                    </DialogDescription>
                  </div>
                </DialogHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card className="bg-slate-900/50 border-slate-800/50 p-6 rounded-3xl space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-3xl font-black text-white">
                        {diagInfo.data.performance.tps}
                      </p>
                      <p className="text-[10px] text-slate-500">
                        Latence: {diagInfo.data.performance.ttft}ms
                      </p>
                    </div>
                  </Card>
                  ...
                  <Card className="bg-slate-900/50 border-slate-800/50 p-6 rounded-3xl space-y-4">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-3xl font-black text-white">
                        {diagInfo.data.overallGrade === 'S' ? '99%' : '85%'}
                      </p>
                ...
                <div className="flex justify-end pt-4 gap-4">
                  <Button
                    ...
                    onClick={() => setDiagInfo((prev) => ({ ...prev, isOpen: false }))}
                  >
                    QUITTER
                  </Button>
                  <Button
                    className="bg-white text-black font-black hover:bg-cyan-400 h-12 px-8 rounded-2xl"
                    onClick={() => runDeepDiagnostic(diagInfo.provider!)}
                  >
                    RELANCER L'AUDIT
                  </Button>
                </div>
...
function ProviderCard({
  ...
  return (
    <motion.div
      layout
      className={cn(
        'group relative bg-[#0f172a]/60 border-slate-800/50 rounded-[32px] overflow-hidden backdrop-blur-xl transition-all duration-500',
        p.enabled ? 'border-cyan-500/30 scale-[1.01] shadow-2xl' : 'opacity-40 grayscale',
      )}
    >
      <div className="p-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                {p.lastDiag && (
                  <Badge className="bg-orange-500 text-[8px] h-4 font-black">
                    GRADE {p.lastDiag.overallGrade}
                  </Badge>
                )}
              </div>
              <div className="flex flex-wrap items-center gap-3 mt-1">
                ...
                <p
                  className={cn(
                    ...
                    p.connectionStatus === 'connected' ? 'text-emerald-400' : 'text-slate-500',
                  )}
                >
                ...
                {p.enabled && p.connectionStatus === 'connected' && (
                  <span className="text-[9px] font-bold text-slate-400">• {p.latency || 0} ms</span>
                )}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-3">
            {p.enabled && p.connectionStatus === 'connected' && (
              <Button
                variant="ghost"
                size="icon"
                className="w-10 h-10 rounded-xl bg-slate-800/50 text-slate-400 hover:text-cyan-400"
                onClick={() => onDiagnostic(p)}
              >
                <Zap className="w-4 h-4" />
              </Button>
            )}
            <Switch
              checked={p.enabled}
              onCheckedChange={(c) => toggleProvider(p.id, c)}
              disabled={!isUnlocked && !isSovereign}
            />
          </div>
        </div>
                  ...
                  (p.isCommitted && p.connectionStatus === 'connected' ? (
                    <>
                      <Button
                        ...
                        onClick={() =>
                          setShowKeys((prev: unknown) => ({ ...prev, [p.id]: !prev[p.id] }))
                        }
                      >
                        {showKeys[p.id] ? (
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-10 w-10 text-slate-500 hover:text-cyan-400"
                        onClick={() =>
                          setProviders((s: unknown) =>
                            s.map((x: unknown) =>
                              x.id === p.id ? { ...x, isCommitted: false } : x,
                            ),
                          )
                        }
                      >
                    ...
                    <Button
                      size="icon"
                      variant="ghost"
                      className="h-12 w-12 text-cyan-400 hover:bg-cyan-500/10"
          ...
          <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-emerald-500 uppercase tracking-widest italic">
                Liaison de Sécurité Maximale Active
              </span>
            </div>
