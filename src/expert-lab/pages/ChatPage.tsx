    ...
    >
      <div className="flex items-center justify-between mb-3 border-b border-slate-800 pb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <div className="flex items-center gap-1">
              ...
              <p className="text-[10px] text-slate-400 uppercase leading-none">
                {data.type === 'voltage_drop' ? 'Chute de Tension' : 'Court-Circuit'}
              </p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Badge
            variant={isConforme ? 'default' : 'destructive'}
            className="text-[10px] font-bold border-none px-1.5 h-5"
          >
            {isConforme ? 'OK' : 'KO'}
          </Badge>
        </div>
...
export default function ChatPage() {
  ...
  const [detachedWidgets, setDetachedWidgets] = useState<
    { data: unknown; x: number; y: number; isMinimized?: boolean }[]
  >([]);
  ...
  const saveRename = async (id: string) => {
    try {
      ...
      toast({ title: 'Erreur lors du renommage' });
    }
  };
  ...
  const groupSessionsByDate = (sessions: ChatSession[]) => {
    ...
    sessions.forEach((s) => {
      ...
      if (isToday(d)) groups["Aujourd'hui"].push(s);
      else if (isYesterday(d)) groups['Hier'].push(s);
      else groups['Précédents'].push(s);
    });
...
> **Note :** La sélectivité ampèremétrique n'est pas totale avec le départ "Atelier".`,
  ...
  const streamChat = async (text: string, att?: typeof attachment) => {
    ...
    try {
      ...
      if (response.modelUsed.includes('DESSIN')) {
        ...
        response.modelUsed.includes('KEBE') ||
        response.modelUsed.includes('expert') ||
        response.modelUsed.includes('Expert')
      ) {
        const expertData = response.data;
        content =
          expertData.answer ||
          (typeof expertData === 'string' ? expertData : JSON.stringify(expertData));
  ...
  const generateVisual = async () => {
    if (!input.trim()) {
      toast({
        title: 'Prompt requis',
        description: "Décrivez l'image que vous voulez générer.",
        variant: 'destructive',
      });
      return;
    }
    ...
    const userMsg: Message = {
      role: 'user',
      content: `🎨 Génère une image visuelle de : ${prompt}`,
    };
      ...
      <div
        className={cn(
          ...
          showSidebar
            ...
            : 'w-0 -translate-x-full overflow-hidden opacity-0 md:opacity-100',
        )}
      >
        {/* Mobile Close Button */}
        <div className="md:hidden absolute -right-10 top-4">
          <Button
            ...
            className="text-white bg-slate-800/80 rounded-r-lg"
          >
            <X className="w-5 h-5" />
          </Button>
        </div>
        ...
        <div className="px-3 py-2 space-y-1 border-b border-slate-800/50 pb-4">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button
                variant="ghost"
                className="w-full justify-start gap-3 text-slate-300 hover:text-cyan-100 hover:bg-cyan-900/10 h-9 px-3 rounded-lg text-sm font-normal transition-colors group"
              >
            ...
            <DropdownMenuContent
              side="right"
              align="start"
              className="bg-[#1e293b] border-slate-700 text-slate-200 w-64 p-2 shadow-2xl"
            >
              <DropdownMenuItem
                onClick={() => (window.location.href = '/expert')}
                className="focus:bg-cyan-900/20 focus:text-cyan-400 cursor-pointer"
              >
              ...
              <DropdownMenuItem
                onClick={() => (window.location.href = '/expert/scanner')}
                className="focus:bg-amber-900/20 focus:text-amber-400 cursor-pointer"
              >
              ...
              <DropdownMenuItem
                onClick={() => (window.location.href = '/ged')}
                className="focus:bg-blue-900/20 focus:text-blue-400 cursor-pointer"
              >
                <ShieldCheck className="w-4 h-4 mr-3 text-blue-500" /> GED PROQUELEC (Souveraine)
              </DropdownMenuItem>
              <DropdownMenuItem
                onClick={() => (window.location.href = '/office/document/new')}
                className="focus:bg-orange-900/20 focus:text-orange-400 cursor-pointer"
              >
          ...
          <DropdownMenu>
            ...
            >
              ...
              <DropdownMenuItem
                onClick={() => (window.location.href = '/expert/history')}
          ...
          <Button
            ...
            variant="ghost"
            className="w-full justify-start gap-3 text-amber-500 hover:text-amber-100 hover:bg-amber-900/10 h-9 px-3 rounded-lg text-sm font-normal transition-colors"
          >
                          ...
                          <>
                            ...
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
                                <Button
                                  variant="ghost"
                                  className="h-6 w-6 p-0 opacity-0 group-hover:opacity-100 hover:bg-slate-700 text-slate-400"
                                  onClick={(e) => e.stopPropagation()}
                                >
                              ...
                              <DropdownMenuContent className="bg-[#1e293b] border-slate-700 text-slate-200">
                                ...
                                >
                                  <Edit3 className="w-4 h-4 mr-2" /> Renommer
                                </DropdownMenuItem>
                                ...
                                <DropdownMenuItem
                                  className="focus:bg-red-900/20 text-red-400 focus:text-red-400"
        ...
        <div className="p-3 border-t border-slate-800 bg-[#0f172a]">
          <DropdownMenu>
              ...
              <div className="flex items-center gap-3 px-2 py-3 rounded-lg hover:bg-slate-800/50 cursor-pointer transition-colors border border-transparent hover:border-slate-700">
                ...
                <div className="flex-1 overflow-hidden">
                  <p className="text-sm font-semibold text-slate-200 truncate">
                    {user?.email || 'Utilisateur Expert'}
                  </p>
                  <p className="text-[10px] text-cyan-500 font-medium">
                    {user?.role === 'admin' ? 'Administrateur' : 'Licence Active'}
                  </p>
                </div>
            ...
            <DropdownMenuContent
              side="right"
              className="bg-[#1e293b] border-slate-700 text-slate-200 w-64 p-3 space-y-3"
            >
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-sm text-slate-300 flex items-center gap-2">
                    <Lock className="w-3 h-3 text-cyan-400" /> Mode Souverain
                  </span>
                  <button
                    onClick={() => setIsSovereignMode(!isSovereignMode)}
                    title={
                      isSovereignMode ? 'Désactiver le Mode Souverain' : 'Activer le Mode Souverain'
                    }
                    className={cn(
                  ...
                  >
                    <div
                      className={cn(
                        ...
                        isSovereignMode ? 'left-4.5' : 'left-0.5',
                      )}
                    />
                  </button>
                </div>
              ...
              <DropdownMenuItem className="focus:bg-slate-800">
                <LucideSettings className="w-4 h-4 mr-2" /> Paramètres Compte
              </DropdownMenuItem>
              <DropdownMenuItem className="focus:bg-red-900/20 text-red-400">
                <XCircle className="w-4 h-4 mr-2" /> Déconnexion
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-[#0f172a] border border-slate-800 rounded-3xl w-full max-w-2xl overflow-hidden shadow-2xl shadow-cyan-500/10">
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[70vh]">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700 space-y-3">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Tous vos échanges et calculs sont stockés localement sur les serveurs PROQUELEC,
                ...
                <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700 space-y-3">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Générez des schémas techniques et des vues architecturales directement par
                    commande vocale ou textuelle.
                  </p>
                </div>
                <div className="p-4 rounded-2xl bg-slate-800/50 border border-slate-700 space-y-3">
                  ...
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Chute de tension, section de câble, bilan de puissance - l'IA exécute les
                    calculs et affiche les widgets interactifs.
                  </p>
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl bg-cyan-950/20 border border-cyan-500/20">
                ...
                <ul className="text-xs text-slate-300 space-y-2 list-disc pl-4">
                  <li>
                    Utilisez le bouton 🪄 pour transformer une idée technique en visuel 3D/2D.
                  </li>
                  <li>
                    Uploadez vos photos de coffrets pour une analyse de conformité automatique.
                  </li>
                  <li>
                    Demandez "Affiche le calcul de chute de tension" pour activer le widget
                    spécialisé.
                  </li>
                </ul>
              </div>
            ...
            <div className="p-6 border-t border-slate-800 flex justify-end">
              <Button
                onClick={() => setShowHelpModal(false)}
                className="bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl px-8"
              >
                J'ai compris
              </Button>
            </div>
              ...
              <SelectContent className="bg-[#1e293b] border-slate-700 text-slate-200">
                {availableModelsList.map((m) => (
                  <SelectItem
                    key={m.id}
                    value={m.id}
                    className="focus:bg-slate-800 cursor-pointer text-xs md:text-sm"
                  >
                    <span className="font-medium text-cyan-400">{m.name}</span>{' '}
                    <span className="text-slate-500 ml-2 hidden md:inline">({m.provider})</span>
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
              ...
              <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-8 animate-in fade-in zoom-in duration-700">
                ...
                <div className="text-center">
                  ...
                  <p className="text-slate-400 text-sm max-w-md mx-auto">
                    L'intelligence artificielle souveraine pour vos audits électriques. Normes
                    certifiées et calculs précis.
                  </p>
                </div>
              ...
              <div key={i} className="group w-full py-8 text-slate-200">
                <div className="flex gap-6">
                  ...
                  <div className="relative flex-1 space-y-2 overflow-hidden">
                          ...
                          img: ({ src, alt, ...props }) => (
                            <img
                              ...
                              onError={(e) => {
                                ...
                                target.onerror = null;
                                target.src =
                                  'https://placehold.co/600x400/0f172a/94a3b8?text=Image+Indisponible';
                              }}
                              {...props}
                            />
                          ),
                    ...
                    {m.widget && (
                      <CalculationWidget data={m.widget} onDetach={() => detachWidget(m.widget)} />
                    )}
                    ...
                    {m.role === 'assistant' && (
                      <div className="flex gap-2 pt-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          variant="ghost"
                          size="icon"
                          className="h-6 w-6 text-slate-500 hover:text-cyan-400 hover:bg-cyan-950/30"
                        >
          ...
          <div className="max-w-[48rem] mx-auto pointer-events-auto">
            ...
            {showSidebar && messages.length > 0 && (
              <div className="flex justify-center mb-4">
                <Badge
                  variant="outline"
                  className="bg-black/40 backdrop-blur-sm border-cyan-500/30 text-cyan-400 text-[10px] px-3 py-1 animate-in fade-in slide-in-from-bottom-2"
                >
            ...
            <div className="bg-[#1e293b]/90 backdrop-blur-xl border border-slate-700/50 shadow-2xl rounded-[26px] p-2 flex flex-col gap-2 transition-all focus-within:ring-1 focus-within:ring-cyan-500/50 focus-within:border-cyan-500/50 focus-within:shadow-[0_0_30px_-5px_rgba(6,182,212,0.15)] relative overflow-hidden">
              ...
              {attachment && (
                <div className="px-4 pt-2 flex items-center gap-3 animate-in slide-in-from-bottom-2">
                  ...
                  <Button
                    size="icon"
                    variant="ghost"
                    className="h-6 w-6 rounded-full hover:bg-slate-700/50"
              ...
              <div className="flex items-end gap-2 px-2">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-9 w-9 rounded-full text-slate-400 hover:text-white hover:bg-white/10 mb-0.5"
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-9 w-9 rounded-full text-purple-400 hover:text-purple-200 hover:bg-purple-900/20 mb-0.5"
                  onClick={generateVisual}
                  title="Générer un visuel (DALL-E)"
                >
                ...
                <div className="flex items-center gap-1 mb-0.5">
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      input.trim()
                        ? 'bg-cyan-500 text-white hover:bg-cyan-400 shadow-[0_0_15px_-3px_rgba(6,182,212,0.5)]'
            ...
            <div className="text-center mt-3">
              <p className="text-[10px] text-slate-500 font-medium">
                IA certifiée PROQUELEC v2.5. Les résultats techniques doivent être validés par un
                ingénieur agréé.
              </p>
            </div>
          </div>
