...
export const Header = ({ solid = false }: HeaderProps) => {
  ...
  const dbSecondaryMenuItems =
    mi
      ?.filter(
        (item) => item.is_active && (item.menu_type === 'secondary' || item.menu_type === 'top'),
      )
    ...
    dbMainMenuItems.length > 0
      ...
      : [
          { label: 'Accueil', path: '/', icon: Home },
          {
            ...
            path: '/about',
            icon: Info,
            submenu: [{ label: 'À propos de PROQUELEC', path: '/about' }],
          },
        ];
    ...
    >
      {isActive && (
        <motion.div
          layoutId="nav-bg"
          className={cn(
            'absolute inset-0 rounded-xl -z-10 border border-white/20',
            isMobile ? 'bg-slate-200/80' : 'bg-white/10',
          )}
      ...
      {children}
      {!isActive && (
        <div className="absolute bottom-1.5 left-4 right-4 h-0.5 bg-white scale-x-0 group-hover:scale-x-100 transition-transform origin-left rounded-full opacity-50" />
      )}
    </Link>
  );
  ...
  const MegaMenuLink = ({ section }: { section: MegaMenuSection }) => {
    ...
    return (
      ...
      >
        ...
        >
          {section.icon && (
            <section.icon className="h-4 w-4 transition-transform group-hover:scale-110" />
          )}
          {section.label}
          <ChevronDown
            className={cn(
      ...
      >
        <div className="container mx-auto px-4 md:px-6 h-full flex items-center">
          <div className="flex items-center justify-between w-full mx-auto">
            ...
            <Link
              to="/"
              className="flex items-center gap-3 sm:gap-5 flex-shrink-0 min-w-0 group relative"
            >
              ...
              <div className="flex flex-col justify-center border-l border-white/20 pl-4 py-1">
                ...
                <p className="text-[10px] sm:text-[11px] font-bold text-white/70 uppercase tracking-[0.1em] mt-1 text-shadow-sm">
                  ...
                  {s?.slogan
                    ...
                    ?.replace('S?n?gal', 'Sénégal') || 'Sécurité · Qualité · Formation'}
                </p>
              </div>
            </Link>
            ...
            <div className="hidden lg:flex items-center gap-2 xl:gap-4 flex-1 justify-end">
                  ...
                  <div className="overflow-trigger">
                    {(() => {
                      ...
                      for (const section of hiddenMegas) {
                        const sectionItems = section.columns
                          .flatMap((col) =>
                            col.items.map((item) => ({
                      ...
                      const totalItems = validGroups.reduce(
                      ...
                      return (
                        ...
                        >
                          <button
                            className={cn(
                              'group relative flex items-center gap-1 px-3 py-2 rounded-xl font-bold text-sm transition-all duration-300',
                              scrolled || solid ? 'text-white' : 'text-white/90 hover:text-white',
                              activeDropdown === '__overflow__' && 'bg-white/10 text-white',
                            )}
                          ...
                          >
                            ...
                            <ChevronDown
                              className={cn(
                                'h-3 w-3 transition-transform duration-300 opacity-50',
                                activeDropdown === '__overflow__' && 'rotate-180',
                              )}
                            />
                          </button>
                          <AnimatePresence>
                            {activeDropdown === '__overflow__' && (
                              <motion.div
                                ...
                                exit={{ opacity: 0, y: 10, scale: 0.95 }}
                                className={cn(
                                  'absolute top-full right-0 mt-2 bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.4)] border border-white/10 p-4 overflow-hidden z-[110] origin-top-right',
                                  colCount === 1
                                    ? 'w-64'
                                    : colCount === 2
                                      ? 'w-[500px]'
                                ...
                                >
                                  {validGroups.map((group, gi) => (
                                    <div key={gi} className="mb-2">
                                      <p className="text-[10px] font-bold uppercase tracking-widest text-blue-400 px-3 py-1.5">
                                        {group.label}
                                      </p>
              ...
              {headerConfig.searchEnabled &&
                (headerConfig.searchPosition === 'center' || !headerConfig.searchPosition) && (
              ...
              <div className="flex items-center gap-2 ml-4 pl-4 border-l border-white/20 flex-shrink-0">
                ...
                {s?.cta_primary_text && (
                  <Link to={s?.cta_primary_url || '/contact'}>
                    <Button className="text-xs md:text-sm px-4 py-2 font-semibold rounded-lg transition-all duration-200 flex items-center gap-2 shadow-sm bg-white/20 hover:bg-white/30 text-white border border-white/20">
                      <Zap className="h-4 w-4" />
                      <span className="hidden xl:inline">{s?.cta_primary_text}</span>
                    </Button>
                  </Link>
                )}
                ...
                {s?.cta_secondary_text && (
                  <Link to={s?.cta_secondary_url || '#'}>
                    ...
                    >
                      ...
                      <span className="hidden xl:inline">{s?.cta_secondary_text}</span>
                    </Button>
                  </Link>
                )}

                {/* Fallback si aucun CTA défini */}
                {!s?.cta_primary_text && !s?.cta_secondary_text && (
                  <Link to="/contact">
                    <Button className="text-xs md:text-sm px-4 py-2 font-semibold rounded-lg transition-all duration-200 flex items-center gap-2 shadow-sm bg-white/20 hover:bg-white/30 text-white border border-white/20">
                ...
                {!isLoading && user ? (
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      <button
                        ...
                        className="text-xs md:text-sm px-3 py-2 font-semibold rounded-lg transition-all duration-200 shadow-md hover:shadow-lg flex items-center gap-2 bg-white text-blue-600"
                        aria-label="Action"
                      >
                  ...
                  <Link to="/connexion">
                    <Button className="text-xs md:text-sm px-4 py-2 font-semibold rounded-lg transition-all duration-200 shadow-md hover:shadow-lg bg-white text-blue-600 hover:bg-blue-50">
                      Connexion
                    </Button>
                  </Link>
                )}
          ...
          <div className="px-4 space-y-1">
            ...
            <div className="pt-3 flex flex-col gap-2">
              ...
              <Link to="/contact" onClick={() => setIsMenuOpen(false)}>
                <Button
                  variant="outline"
                  className="w-full justify-center gap-2 rounded-2xl py-4 border border-proqblue bg-white text-proqblue hover:bg-slate-50"
                >
                  <Phone className="h-4 w-4" />
                  Nous Contacter
                </Button>
