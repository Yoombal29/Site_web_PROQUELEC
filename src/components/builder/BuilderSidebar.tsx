 ...
 * Replaced by GodToolbox in src/components/god-builder/.
...
const DraggableItem = React.memo(
  ({ type, label, icon }: { type: string; label: string; icon?: React.ReactNode }) => {
  ...
  return (
    ...
    >
      ...
      <span className="text-[10px] text-slate-400 capitalize">
        {template.block.type} • {new Date(template.createdAt).toLocaleDateString()}
      </span>
    </div>
  );
});
...
export const BuilderSidebar: React.FC<BuilderSidebarProps> = ({ templates, deleteTemplate }) => {
  return (
    <aside
      aria-label="Bibliothèque de blocs"
      className="w-72 bg-white border-r border-slate-200 flex flex-col shadow-sm z-10 shrink-0"
    >
      ...
      <Tabs defaultValue="elements" className="flex-1 flex flex-col overflow-hidden">
        ...
        <TabsContent value="elements" className="flex-1 overflow-y-auto p-4 space-y-2">
          <div className="uppercase text-[10px] font-bold text-slate-400 mb-2 tracking-wider">
            Base
          </div>
          ...
          <div className="uppercase text-[10px] font-bold text-blue-600 mb-2 mt-5 tracking-wider border-t border-slate-100 pt-4">
            📐 Layout & Structure
          </div>
          <DraggableItem type="header-builder" label="Header Builder" icon={<Box size={16} />} />
          <DraggableItem type="footer-builder" label="Footer Builder" icon={<Box size={16} />} />
