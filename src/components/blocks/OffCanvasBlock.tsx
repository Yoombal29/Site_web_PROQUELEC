...
export const OffCanvasPanelBlock = (props: any) => {
  ...
  const isVisible = !enabled && (controlledVisible !== undefined ? controlledVisible : internalVisible);
  const toggle = () => {
    if (enabled) return;
  ...
  return (<div ref={(r: any) => { if (r) connect(drag(r)); }} style={{ position: 'relative', ...u.style }} className={'proquelec-builder-node ' + u.className}>
    <button onClick={toggle} style={{ padding: '8px 16px', background: '#2563eb', color: 'white', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: enabled ? 'grab' : 'pointer' }}>
      ☰ Ouvrir
    </button>
    {isVisible && <div onClick={closeOnOverlay ? toggle : undefined} style={{ position: 'fixed', inset: 0, background: overlayColor, zIndex: 9999 }} />}
    {!enabled && <div style={{ position: 'fixed', ...sizeStyles[position] || sizeStyles.right, ...slideStyles[position] || slideStyles.right, background: bgColor, boxShadow: '0 0 30px rgba(0,0,0,0.15)', zIndex: 10000, transition: 'transform 0.3s ease', overflowY: 'auto' }}>
...
export const OffCanvasToggleBlock = (props: any) => {
  ...
  return (<button ref={(r: any) => { if (r) connect(drag(r)); }} onClick={findAndToggle}
    style={{ background: bgColor, color: textColor, border: 'none', padding: '8px 16px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: enabled ? 'grab' : 'pointer', width: fullWidth ? '100%' : 'auto', transition: 'all 0.2s', ...u.style }}
