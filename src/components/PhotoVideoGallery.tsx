...
import {
  ...
  Info, X, ChevronRight, Zap, ShieldCheck } from
'lucide-react';
...
export function PhotoVideoGallery() {
  ...
  useEffect(() => {
    const fetchGallery = async () => {
      setLoading(true);
      try {
        const data = await apiFetch<MediaItem[]>('/api/gallery-items');
    ...
    <div className="space-y-12 py-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        <div
          ...
          onClick={() => setSelectedItem(item)}>
            ...
            <div
            className={`relative aspect-[4/5] overflow-hidden rounded-[2.5rem] bg-gray-100 shadow-xl transition-all duration-700 group-hover:-translate-y-4 group-hover:shadow-2xl animate-in fade-in slide-in-from-bottom-4 fill-mode-backwards [animation-delay:${idx * 100}ms]`}>
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-10 animate-in fade-in zoom-in duration-300">
          ...
          <div className="relative w-full max-w-7xl h-full flex flex-col md:flex-row gap-10 bg-white/5 rounded-[3rem] border border-white/10 overflow-hidden shadow-[0_0_100px_rgba(35,118,223,0.3)]">
            ...
            <img src={selectedItem.url} alt={selectedItem.title} className="w-full h-full object-contain" loading="lazy" /> :
            ...
            <video src={selectedItem.url} controls className="w-full h-full" autoPlay />
              ...
              <TooltipProvider>
                ...
                __html: (selectedItem.hotspots || []).map((hs, i) => `
                  .gallery-hotspot-${i} { left: ${hs.x}%; top: ${hs.y}%; }
                `).join('\n')
              }} />
                {selectedItem.hotspots?.map((hs, i) =>
              <div
                key={i}
                className={`absolute group/hotspot cursor-help gallery-hotspot-${i}`}>
                
                    <Tooltip open={activeHotspot === i}>
                      <TooltipTrigger asChild>
                        <button
                      aria-label={`Voir détail: ${hs.title}`}
