...
export default function AdminPartnersPanel() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (editingId) {
            ...
            else toast.success("Partenaire ajouté");
        }
    ...
    return (
        <div className="space-y-8">
            <div className="flex justify-between items-center border-b pb-4">
                <h2 className="text-2xl font-bold text-proqblue">Gestion des Partenaires</h2>
                <Badge variant="outline">{partners.length} partenaires</Badge>
            </div>

            {/* Formulaire */}
            <div className="bg-slate-50 p-6 rounded-lg border">
                <h3 className="font-semibold mb-4 text-lg">{editingId ? 'Modifier un partenaire' : 'Ajouter un nouveau partenaire'}</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {partners.map(p => (
                    <div key={p.id} className="bg-white p-4 rounded-lg border shadow-sm flex flex-col gap-3 group relative">
                        <div className="flex justify-between items-start">
                            <div className="h-12 w-24 bg-gray-50 rounded flex items-center justify-center p-1 border">
