...
import { Loader2, Plus, Trash2, Save, Image as ImageIcon, Zap, Info, X } from "lucide-react";
...
export default function AdminGalleryPanel() {
    ...
    const fetchItems = async () => {
        ...
        if (error) toast.error("Erreur chargement galerie");
        else setItems(data || []);
        setLoading(false);
    };
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const { error } = editingId
            ? await (supabase as any).from('gallery_items').update(payload).eq('id', editingId)
            : await (supabase as any).from('gallery_items').insert(payload);
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="space-y-4">
                            <div>
                                ...
                                <Input value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} placeholder="Ex: Poste de transformation T2" />
                            </div>
                            <div>
                                <Label className="font-bold">Description Éducative</Label>
                                <Textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Décrivez le projet et son intérêt pédagogique..." rows={4} />
                            </div>

                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <Label className="font-bold">Catégorie</Label>
                                    <Input value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} placeholder="formations, projets, installations" />
                                </div>
                                <div>
                            ...
                            {form.url && form.type === 'photo' && (
                                <div className="space-y-4 p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                    ...
                                    <div className="relative group cursor-crosshair overflow-hidden rounded-xl bg-black shadow-inner" onClick={handleImageClick}>
                                        <img ref={imgRef} src={form.url} className="w-full opacity-80" alt="Previsualisation" />
                                        {form.hotspots?.map((hs, i) => (
                                            <div
                                                key={i}
                                                className="absolute w-6 h-6 -ml-3 -mt-3 bg-proqblue border-2 border-white rounded-full flex items-center justify-center text-[10px] text-white font-bold shadow-lg"
                                    ...
                                    <div className="space-y-4 max-h-64 overflow-y-auto pr-2">
                                        {form.hotspots?.map((hs, i) => (
                                            <div key={i} className="bg-white p-4 rounded-xl border relative shadow-sm animate-in slide-in-from-right-2">
                                                <button onClick={() => removeHotspot(i)} className="absolute top-2 right-2 text-red-500 hover:bg-red-50 p-1 rounded-md">
                                                    <Trash2 size={16} />
                                                </button>
                                                <Input
                        ...
                        {items.map(item => (
                            <div key={item.id} className="group relative bg-white rounded-3xl overflow-hidden border shadow-sm hover:shadow-xl transition-all h-80">
                                ...
                                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                    ...
                                    <Button size="icon" className="bg-red-500 text-white hover:bg-red-600 rounded-xl shadow-lg" onClick={async () => {
                                        if (confirm("Supprimer ?")) {
                                            await (supabase as any).from('gallery_items').delete().eq('id', item.id);
                                            fetchItems();
                                        }
                                    }}>
                                        <Trash2 size={18} />
