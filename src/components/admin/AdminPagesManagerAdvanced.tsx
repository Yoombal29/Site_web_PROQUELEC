...
import { AlertCircle, Plus, Trash2, Save, Eye } from 'lucide-react';
...
const AdminPagesManagerAdvanced: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      try {
        // Use local API
        const data = await apiFetch<Page[]>('/api/pages');
        setPages(data || []);
      } catch (error) {
  ...
  const handleSavePage = async () => {
    ...
    try {
      ...
      if (isCreating) {
        // Créer une nouvelle page
        const newPageData = { ...pageData, slug: editForm.slug.trim().toLowerCase() };
        ...
        await apiFetch(`/api/pages/${selectedPage.id}`, {
          method: 'PUT',
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent className="space-y-2 max-h-96 overflow-y-auto">
            ...
            <div
              ...
              className={`p-3 rounded cursor-pointer transition ${selectedPage?.id === page.id ?
