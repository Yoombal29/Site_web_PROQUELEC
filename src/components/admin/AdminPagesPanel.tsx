...
const AdminPagesPanel: React.FC = () => {
  ...
  const { data: existingMenuItems = [] } = useMenuItems();
  ...
  const handleApplyTheme = async (theme: CmsTheme) => {
    if (
      !window.confirm(
        `Appliquer le thème "${theme.display_name}" ? Vos réglages de design actuels seront mis à jour.`,
      )
    )
      return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingPage) {
        ...
        const createdPage = (await createPageMutation.mutateAsync(validatedData)) as any;

        // Si l'option "Ajouter au menu" est cochée, créer l'entrée de menu
        if (addToMenu && createdPage?.id) {
          try {
            await createMenuItemMutation.mutateAsync({
              title: menuLabel || formData.title,
              label: menuLabel || formData.title,
              url: '/' + formData.slug,
              menu_type: menuType,
              menu_order: Math.floor(Number(formData.menu_order) || 0),
              parent_id: menuParentId,
              is_active: true,
              linked_page_id: createdPage.id,
            });
            toast({
              title: '✅ Page & Menu créés',
              description: 'La page a été créée et ajoutée au menu avec succès.',
            });
          } catch (menuErr) {
            console.error('Menu creation error:', menuErr);
            toast({
              title: '⚠️ Page créée, erreur menu',
              description: "La page a été créée mais l'entrée de menu n'a pas pu être ajoutée.",
              variant: 'destructive',
            });
          }
        } else {
  ...
  const removeCategory = (category: string) => {
    setFormData((prev) => ({
      ...prev,
      categories: (Array.isArray(prev.categories) ? prev.categories : []).filter(
        (c) => c !== category,
      ),
    }));
  };
            ...
            <div className="flex gap-2 items-center">
              <Button
                ...
                onClick={async () => {
                  if (
                    !window.confirm(
                      "Initialiser la page d'accueil dans le BE Builder avec les 6 sections originales (Carrousel, Audience, Vision, Stats, News, Partenaires) ?",
                    )
                  )
                    return;
                  try {
                    const res = await fetch('/api/admin/seed-homepage', {
                      method: 'POST',
                    ...
                    if (data.success) {
                      toast({
                        title: "✅ Page d'accueil initialisée !",
                        description: `${data.blocksCount} sections injectées. Ouvrez la page "home" dans le BE Builder.`,
                      });
                      refetch();
                    } else {
              ...
              >
                <DialogHeader>
                  ...
                  <p id="page-editor-description" className="text-sm text-gray-600">
                    {editingPage
                      ? 'Modifiez les paramètres et le contenu de la page.'
                ...
                <div className="mb-4 p-3 bg-blue-50 rounded-md border">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span
                      className={`text-sm font-bold ${seoScore >= 70 ? 'text-green-600' : seoScore >= 50 ? 'text-yellow-600' : 'text-red-600'}`}
                    >
                      {seoScore}/100
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`text-sm font-bold ${readabilityScore >= 70 ? 'text-green-600' : readabilityScore >= 50 ? 'text-yellow-600' : 'text-red-600'}`}
                    >
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, meta_robots: e.target.value }))
                          }
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, language_code: e.target.value }))
                          }
                          className="w-full p-2 border border-gray-300 rounded-md"
                      ...
                      <div className="flex items-center space-x-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) =>
                            setFormData((prev) => ({ ...prev, show_hero: checked }))
                          }
                        />
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, header_html: e.target.value }))
                          }
                      ...
                      <div>
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, footer_html: e.target.value }))
                          }
                          className="min-h-[150px] font-mono text-sm"
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <MediaSelector
                          ...
                          onSelect={(url) =>
                            setFormData((prev) => ({ ...prev, hero_background_image: url }))
                          }
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, hero_cta_text: e.target.value }))
                          }
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, hero_cta_link: e.target.value }))
                          }
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setFormData((prev) => ({ ...prev, unpublish_date: e.target.value }))
                          }
                        />
                      </div>

                      {/* ── Ajouter au menu (uniquement pour les nouvelles pages) ── */}
                      {!editingPage && (
                        <div className="border rounded-lg p-4 bg-slate-50 space-y-3">
                          ...
                          {addToMenu && (
                            <div className="space-y-3 pl-2 border-l-2 border-blue-200">
                              <div>
                                <Label htmlFor="menuLabel">Libellé du menu</Label>
                                <Input
                                  id="menuLabel"
                                  value={menuLabel}
                              ...
                              <div>
                                <Label htmlFor="menuType">Type de menu</Label>
                                <select
                                  id="menuType"
                                  value={menuType}
                                  onChange={(e) => setMenuType(e.target.value as 'main' | 'footer')}
                                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                                >
                                  <option value="main">Menu principal (header)</option>
                                  <option value="footer">Menu pied de page (footer)</option>
                                </select>
                              </div>
                              <div>
                                <Label htmlFor="menuParent">Menu parent (optionnel)</Label>
                                <select
                                  id="menuParent"
                                  value={menuParentId || ''}
                                  onChange={(e) => setMenuParentId(e.target.value || null)}
                                  className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
                                >
                                  <option value="">Aucun (menu racine)</option>
                                  {(existingMenuItems as any[])
                                    .filter(
                                      (item: any) => item.menu_type === menuType && !item.parent_id,
                                    )
                                    .map((item: any) => (
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2 mb-2">
                          {(Array.isArray(formData.categories) ? formData.categories : []).map(
                            (category, index) => (
                              <span
                                key={index}
              ...
              {loadingPlugins ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {availablePlugins?.map((plugin) => (
                    <div
                      key={plugin.id}
                      className="bg-white border rounded-lg p-5 flex flex-col justify-between hover:border-blue-200 transition-colors"
                    >
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mb-3">
                          v{plugin.version} • par PROQUELEC
                        </p>
              ...
              <div className="p-4 border-b bg-gray-50">
                ...
                {selectedPages.length > 0 && (
                  <div className="flex items-center gap-2 mt-3">
                    ...
                    <Button onClick={() => setSelectedPages([])} variant="outline" size="sm">
                      Annuler
                    </Button>
                  </div>
                )}
                      ...
                      >
                        <div className="flex items-start gap-3 flex-1">
                          ...
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-gray-600">
                              Slug: {page?.slug || 'sans-slug'}
                            </p>
                            <div className="flex items-center gap-4 mt-1">
                              <span
                                className={`text-xs px-2 py-1 rounded ${page?.is_published ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}
                              >
                        ...
                        <div className="flex gap-2">
                          ...
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              window.open(`/admin/builder/${page.slug || page.id}`, '_blank')
                            }
          ...
          <Dialog open={showPreview} onOpenChange={setShowPreview}>
            ...
            >
              ...
              <div className="flex-1 overflow-hidden">
                <div
                  className={`mx-auto border rounded-lg overflow-hidden h-[70vh] bg-white ${
                    previewMode === 'desktop'
                      ? 'w-full max-w-4xl'
                      : previewMode === 'tablet'
                        ? 'w-full max-w-md'
