...
export default function AdminBlogPanel() {
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center p-8"><Loader2 className="animate-spin h-8 w-8 text-proqblue" /></div>;
  }
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        <h3 className="text-lg font-semibold text-proqblue-dark">Gestion des articles</h3>
        <div className="flex flex-col md:flex-row gap-2 md:items-center">
          <input
            ...
            className="px-3 py-2 border rounded w-full max-w-xs text-sm"
          />
      ...
      <div className="space-y-4">
        {paginatedPosts && paginatedPosts.length > 0 ? (
            ...
            <div key={post.id} className="flex items-center justify-between p-4 border rounded-lg bg-white shadow-sm">
              <div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  {post.blog_categories?.name && <span className="bg-gray-200 px-2 py-0.5 rounded-full text-xs">{post.blog_categories.name}</span>}
                </div>
          ...
          <p className="text-center text-gray-500 py-6">Aucun article pour le moment.</p>
        )}
        {totalPages > 1 && (
