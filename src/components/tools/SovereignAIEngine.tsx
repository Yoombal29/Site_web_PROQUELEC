...
import {
    Brain, ShieldAlert, BadgeCheck, Send,
    RotateCcw, Info, Lock, ChevronRight,
    ShieldCheck, AlertTriangle, Scale
} from 'lucide-react';
                ...
                {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center px-12 space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-lg mt-4">
                            {['Limite chute de tension ?', 'Intervention en zone BT ?'].map(t => (
                                <button
                                    key={t}
                                    onClick={() => setQuery(t)}
                                    className="px-4 py-3 bg-[#0a231b] border border-emerald-900/50 rounded-xl text-xs font-bold text-emerald-500/80 hover:border-emerald-500/40 hover:text-emerald-400 transition-all text-left flex items-center justify-between"
                                >
                                    ...
                                    {msg.articles.map((art: any, i: number) => (
                                        <div key={i} className="flex flex-col gap-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {art.alfresco_node_id && (
                                                    <a
                                                        href={`http://localhost:8180/share/page/document-details?nodeRef=${art.alfresco_node_id}`}
                                                        target="_blank"
                                                        rel="noopener noreferrer"
                                                        className="flex items-center gap-1.5 px-2 py-1 bg-emerald-500/10 border border-emerald-500/30 rounded text-[9px] font-black text-emerald-400 hover:bg-emerald-500/20 transition-all uppercase tracking-tighter"
                                                    >
                                                        <ShieldAlert className="w-3 h-3 text-orange-400" />
                                                        Voir Document Souverain (GED)
                                                    </a>
                                                )}
                                            </div>
                                        </div>
