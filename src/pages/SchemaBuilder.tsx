...
import { AlertCircle, CheckCircle2, Zap, RotateCcw, RotateCw } from 'lucide-react';
...
export default function SchemaBuilder() {
  ...
  useEffect(() => {
    ...
    if (rubriqueParam) {
      const selectedRubrique = rubriqueRegistry.get(rubriqueParam as unknown);
      if (selectedRubrique) {
  ...
  // Gestion des charges
  const handleAddCharge = (nodeId: string, chargeData: unknown) => {
    graphStore.addCharge(nodeId, chargeData);
  };

  const handleUpdateCharge = (nodeId: string, chargeId: string, updates: unknown) => {
        ...
        'bg-gradient-to-br from-red-900/20 to-rose-900/20 border-red-500/30'}`
            ...
            <AlertCircle className="w-6 h-6 text-red-400 flex-shrink-0 mt-0.5" />
              ...
              <div>
                      ...
                      <ul className="space-y-1">
                        {validationResult.warnings.map((warn: unknown, i: number) =>
        ...
        <div className="mt-8 rounded-xl border border-slate-700/50 bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm p-6 transition-all duration-300">
          ...
          <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-6">
                    {calculationResult.metrics && calculationResult.metrics.map((metric: unknown, i: number) =>
            ...
            <div className="space-y-3">
                      ...
                      {calculationResult.tronçons.map((tronçon: unknown, i: number) =>
