...
import { Home, Shield, Wrench, Cpu, ChevronRight, Globe, Lock, FileCode } from 'lucide-react';
      ...
      {items.map((item: any, index: number) => (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm text-slate-300 group-hover/item:text-white transition-colors capitalize">
              {item.label}
            </span>
          </div>
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 py-16 px-4 md:py-24">
      <div className="max-w-7xl mx-auto space-y-16">
        ...
        <div className="relative text-center space-y-4">
          <Badge
            variant="outline"
            className="text-[10px] border-proqblue/30 text-proqblue px-4 py-1 uppercase tracking-widest font-black animate-pulse"
          >
          ...
          <h1 className="text-4xl md:text-7xl font-black text-white uppercase tracking-tighter leading-tight italic">
            Plan du{' '}
            <span className="text-proqblue not-italic tracking-normal text-stroke-white">
              Système
            </span>
          </h1>
          <p className="max-w-2xl mx-auto text-slate-400 text-lg font-light leading-relaxed">
            Cartographie complète de l'écosystème numérique de{' '}
            <span className="text-white font-bold tracking-widest uppercase">Proquelec</span>.
            Accédez rapidement à toutes nos plateformes et outils.
          </p>
        ...
        <div className="pt-20 border-t border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6 opacity-40">
          <div className="flex items-center gap-6 text-[10px] font-black tracking-widest uppercase">
            <span className="flex items-center gap-2">
              <Globe className="w-3 h-3" /> Navigation Chiffrée
            </span>
            <span className="flex items-center gap-2">
              <Shield className="w-3 h-3" /> Accès Sécurisé
            </span>
          </div>
