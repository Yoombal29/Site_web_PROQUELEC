...
**VoltageDropCalculator** is an enterprise-grade electrical engineering tool that calculates voltage drop in electrical installations with strict compliance to NF C 15-100 (French electrical standard).

### Key Characteristics
- **Domain**: Electrical Engineering
- **Standard**: NF C 15-100 (French norm)
...
|---------|---------------------|-------------------|
| Normative Compliance | ✅ Strict NF C 15-100 | ❌ Generic formulas |
...
│  • calculateVoltageDrop() - Core NF C 15-100 formula     │
...
┌──────────────────▼──────────────────────────────────────┐
│             Utility Layer                                │
│  • normativeConstants.ts - Lookup tables (NF C 15-100)   │
...
graph LR
    ...
    C["Calculation Engine<br/>(NF C 15-100 formula)"]
...
### 1. **Core Calculation (NF C 15-100)**
```typescript
...
- **Voltage Drop Limits per NF C 15-100:**
  - Main circuit: 3% max
  - Final circuit: 5% max
...
interface VoltageDropResult {
  ...
  isCompliant: boolean;       // Meets NF C 15-100
...
Validates if section is in NF C 15-100 normalized list.
...
Returns max allowed voltage drop per NF C 15-100.
